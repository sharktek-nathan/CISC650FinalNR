$(document).ready(function() {
	...
	/* --------  Initialize Content --------  */
	$('#available_numbers').html('<option value="null">Select a fax number</option>');
	...
	$('#state').on('change', function() {
		$('#area_code').html('<option value="unselected">Select an area code</option>');
		$('.area_hide').show();
		$('#city').html('');
	...
	$('#area_code').on('change', function() {
		...
		$.getJSON( "/setup/did_search/", { area: area_code} )
			.done(function( data ) {
				did_search = data;
			  	if(did_search.length === 0) {
					$('#city').html('<option value="">No Results Found</option>');
				} else {
	...
	$('#city').on('change', function() {
		$('#available_numbers').html('<option selected="selected">Select a fax number</option>');
		$('.available_numbers_hide').show();
		var city = $(this).val();
		for(var i = 0; i < did_search[city].length; i++){
			var formatted = did_search[city][i].substr(5, 3) + '.' + did_search[city][i].substr(8, 3) + '.' + did_search[city][i].substr(11,4);
			$('#available_numbers').append('<option value="' + did_search[city][i] +'">' + formatted + '</option>');
		}
	});
	...
	$("#did-form").validate({
		...
		invalidHandler: function(event, validator) {
			var errors = validator.numberOfInvalids();
				if (errors) {
					var message = 'You must select a number to continue.'
					$("#submit_did_check").html(message);
					$("#submit_did_check").show();
				} else {
					$("#submit_did_check").hide();
				}
		},
		...
		invalidHandler: function(event, validator) {
			var errors = validator.numberOfInvalids();
				if (errors) {
					...
					$("#submit_port_check").html(message);
					$("#submit_port_check").show();
				} else {
					$("#submit_port_check").hide();
				}
		},
	  	rules: {
		current_number: {
		...
		invalidHandler: function(event, validator) {
			var errors = validator.numberOfInvalids();
				if (errors) {
					...
					? 'You missed 1 field. Please go back and fix the error.'
					: 'You missed ' + errors + ' fields.  Please go back and fix the errors.';
					$("#submit_check_two").html(message);
					$("#submit_check_two").show();
				} else {
					$("#submit_check_two").hide();
				}
			},
		submitHandler: function(form) {
			$.ajax({
