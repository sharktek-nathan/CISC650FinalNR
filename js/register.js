...
//        {
            $('#available_numbers').popover({ trigger: "manual", html: true, placement: 'left' });
//        }
    
        // Applies the number mask to the current_number field in Step One Port
        $("#current_number").mask('(000) 000-0000');
    
        // Applies the number mask to the phone_number field in Step Two
	...
	// On selection of a state:
        $('#state').on('change', function() { 
            ...
            $('#to_step_two').hide();
            $('#no_number_continue_button').hide(); 
        ...
        $.getJSON( "/app/setup/did_search/", { city: selected_city, state: selected_state } )
            .done(function( data ) {
                ...
                if(did_search.length === 0) {
                    ...
                    $('#no_number_continue_button').show();
        ...
        $('#available_numbers').on('change', function() { 
            ...
            if(newDID.length === 15) {
                ...
                // Set value of the hidden 'select_did' field in the form
                $('#select_did').val(newDID);
	...
	$('#back_step_one').on('click', function() { 		
            var val = $('input:radio[name=number_status_radio]:checked').val();
            show_step1(val);
	...
	$('input:radio[name=number_status_radio]').click(function() {
            var val = $('input:radio[name=number_status_radio]:checked').val();
            show_step1(val);
	});
	
        // Globals, set by the to_step_two button or no_number_continue_button when either is pressed
        var did;
        var did_text;
        
        // Proceeds with registration when user selected a number and clicks to Step Two
        $('#to_step_two').on('click', function(){
            did = $("#select_did").val();
            did_text = $('#available_numbers option:selected').text();  
            $('#available_numbers').popover('hide');
        });
        
        // Proceeds with registration when user is continuing without selecting a number and clicks to Step Two
        $('#no_number_continue_button').on('click', function(){
            did = 'unv_00000000000';
            $('#select_did').val(did);
            did_text = $('#city').val() + ' Number';
            $('#available_numbers').popover('hide');
        });
        
        // Handles the Step One New Number form
	$("#did-form").validate({          
            submitHandler: function(form) {                
                
                // Set the hidden 'did' field on Step Two
                $("#did").val($("#select_did").val());
        ...
        // Handles the Step One Port Number form
	$("#port-form").validate({
            errorClass: "invalid",
            validClass: "valid",
            ignore: ".ignore",                        
            submitHandler: function(form) {
                $("#did").val('gen_00000000000');
                var current_number_raw = strip_non_digit( $('#current_number').val() );
                var current_number = current_number_raw.substring(0, 3) + '.' + current_number_raw.substring(3, 6) + '.' + current_number_raw.substring(6, 10);
                $.ajax({
                    url: "/app/setup/set_session/",
                    type:"POST",
                    dataType:"json",
                    data: {                        
                        ...
                        current_provider : $('#current_provider').val(),                       
                        did : 'gen_00000000000',
                        last_step : 2,
                        service: 'Port Number'
                    }
            ...
            invalidHandler: function(event, validator) {},
            rules: {
                current_number: {required: true, phoneUS: true}, 
                current_provider: {maxlength: 30}
            }
	});
	
        // Handles the Step Two form
	$("#register-form").validate({
            ...
            submitHandler: function(form) {
                ...
                var phone_number = strip_non_digit( $('#phone_number').val() );
                $.ajax({
                    url: "/app/setup/set_session/",
                    type:"POST",
                    data: { 
                        ...
                        pricing_text : $('#pricing_id option:selected').text()
                    },
                    success: function(data, textStatus, jqXHR) {
                        $('#finish_step_two').hide();
                        form.submit();
                    },
           ...
           invalidHandler: function(event, validator) {},
            rules: {
                ...
                new_password: {required: true, minlength: 5, maxlength: 30},
                confirm_password: {required: true, equalTo: "#new_password", minlength: 5, maxlength: 30},
        ...
        function strip_non_digit(number) {
...
var usStates = [
	...
	{ name: 'Toll Free', abbreviation: 'TF'},
...
var usAreaCodes = [
	...
	{ area_code: '800', state: 'TF'},
	{ area_code: '855', state: 'TF'},
	{ area_code: '866', state: 'TF'},
	{ area_code: '888', state: 'TF'},
